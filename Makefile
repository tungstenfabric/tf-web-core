...
define fetch_xsd_schemas
	 wget -qO - https://github.com/tungstenfabric/tf-api-client/tree/${BRANCH}/schema | \
	 awk -F '[<>]' '/.*\.xsd/ { for(i=0;i<=NF;i++) { if($$i ~ /[^<>].*\.xsd$$/) { print $$i; break; } } }' | \
	 while read -r line; do wget https://raw.githubusercontent.com/tungstenfabric/tf-api-client/$(BRANCH)/schema/$$line; done;
endef

$(WEBUISERVER):
	if [ ! -d ../$(WEBUISERVER) ]; then git clone git@github.com:tungstenfabric/tf-web-core.git ../$(WEBUISERVER); else cd ../$(WEBUISERVER) && touch testFile && git stash; git pull --rebase; git stash pop; rm testFile; fi
